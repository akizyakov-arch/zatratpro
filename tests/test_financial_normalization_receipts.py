...
CASES_PATH = Path(__file__).with_name("golden").joinpath("receipt_financial_cases.json")
...
class ReceiptFinancialNormalizationGoldenTest(unittest.TestCase):
    maxDiff = None
    ...
    def test_receipt_financial_golden_cases(self) -> None:
        for case in self.cases:
            with self.subTest(case=case["id"]):
                ...
                self.assertEqual(
                    ...
                    f'{case["id"]}: unexpected provenance source',
                )
                self.assertEqual(
                    list(result.provenance.warnings),
                    expected_provenance.get("warnings", []),
                    f'{case["id"]}: unexpected provenance warnings',
                )
                self.assertEqual(
